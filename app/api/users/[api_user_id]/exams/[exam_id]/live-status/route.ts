...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${apiUrl}/api/users/${api_user_id}/exams/${exam_id}/live-status`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('Error fetching exam live status:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to fetch exam live status',
      },
      { status: 500 },
    );
  }
