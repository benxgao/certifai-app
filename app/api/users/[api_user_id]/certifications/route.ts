...
export async function DELETE(
  ...
  try {
    ...
    // Get cert_id from query parameters
    const searchParams = request.nextUrl.searchParams;
    ...
    const USER_CERTIFICATIONS_API_URL = `${process.env.NEXT_PUBLIC_SERVER_API_URL}/api/users/${api_user_id}/certifications/${cert_id}`;
    ...
    if (!response.ok) {
      const errorData = await response.text();
      console.error(
        `Failed to delete certification ${cert_id} for user ${api_user_id}:`,
        response.status,
        errorData,
      );
      return NextResponse.json(
        { message: `Failed to delete certification`, error: errorData },
        { status: response.status },
      );
    }
    ...
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
    return NextResponse.json(
      { message: 'Error deleting user certification', error: errorMessage },
      { status: 500 },
    );
  }
}
