...
3. Execute [`v1/cli/init.md`](v1/cli/init.md) **as a rollout plan** — it generates all folders and files inside `v1/spec/` for your repo (this is the genesis/recursive self-generation step).
4. Populate the domain scaffolds in `v1/spec/` (`architecture/`, `api/`, `state/`, `data/`, `style/`, `security/`, `performance/`, `testing/`, `product/`) with `[project: fill in]` placeholders replaced by your project's actual content.
5. Register every new doc in [`v1/spec/ai/assistant-context-index.md`](v1/spec/ai/assistant-context-index.md).
6. Validate copy-readiness with the scans below.
...
# 2. Version scope: no internal reference escapes intent-ops/v1/
grep -rn "\.\./\.\./\.\./" intent-ops/v1/          # must return nothing
