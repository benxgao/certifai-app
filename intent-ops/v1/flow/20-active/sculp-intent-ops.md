...
The package is now versioned under `v1/` (flow, spec, cli). This is a deliberate architecture decision: `intent-ops` is expected to become an isolated project and to be upgraded to `v2`/`v3` in the future. Per the user-confirmed version model (2026-08-27), each new version carries a changelog of changes from the previous version and a migration rollout plan/instruction document; after migration, the previous version's flow/spec folders are emptied and the new version becomes the primary source of truth. For migration to stay manageable, `v1` must be fully self-contained: every internal reference resolves inside `v1/`, nothing outside the package is required at runtime, and the version model is documented.
...
- Versioned layout `v1/` (flow / spec / cli) — the container for the current version; future versions (v2, v3, …) are planned per the confirmed version model (per-version changelog + migration rollout plan/instruction; previous versions emptied after migration).
...
- `README.md` still describes the old flat layout (`ops-flow` / `ops-spec`) — it neither matches the `v1/flow` / `v1/spec` structure nor documents the version model (per-version changelog + migration rollout plan/instruction; previous versions emptied after migration).
...
- [ ] If links or references escape the `v1/` scope (absolute paths, cross-version links), future-version derivation is no longer a trivial copy of a self-contained `v1/`.
...
- Creating the future `v2/` folder — this rollout only makes `v1` self-contained so a future v2 can be derived from it (per the confirmed version model: changelog + migration rollout plan/instruction doc per version, previous versions emptied after migration — documented, not executed). A v1 → v2 migration checklist is documented, not executed.
- Changing the methodology itself (principles, lane structure, lane transition criteria in `v1/flow/README.md`).
- Modifying the source folders (`ai_oriented_kanban/`, `docs/` at repository root) — extraction is one-way; user-confirmed (2026-08-27) that `intent-ops` will be relocated into its own standalone repo and the source folders stay project-local.
- Project-specific docs for the source project (the package ships scaffolds that are best-effort populated from a target project's scan and refined by a human — user-confirmed scan→build depth, 2026-08-27).
...
| `docs/ai/repo-map.md` (repo root)                                                                                                                         | System-boundary/route-map structure — reusable as a skeleton that is best-effort populated from the target project's scan (no certifai paths).                                                                                                                                                           |
...
| `intent-ops/v1/cli/init.md`                                                                 | The genesis rollout plan being authored — a conformant rollout plan whose execution generates all folders and files inside `v1/spec/` (best-effort populated from the target project's scan, refined by a human); defines what "initialized repo" means. |
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Adopt a versioned layout: `v1/flow`, `v1/spec`, `v1/cli`; each future version (v2, v3, …) ships a changelog vs the previous version + a migration rollout plan/instruction doc; after migration the previous version's flow/spec folders are emptied and the new version becomes the primary source of truth | `intent-ops/README.md` (copy promise), user decision (2026-08-27: versioning cadence confirmed)                                                                 | Sufficient          | No                       | Create `v1/README.md` (manifest + migration checklist + changelog convention); document the version model in `intent-ops/README.md` and `instructions.md` |
...
| `intent-ops/v1/README.md`                                                                                                                               | Version-scoped manifest: what `v1/` contains (flow, spec, cli), how it relates to the package root, the version model (per-version changelog + migration rollout plan/instruction; previous versions emptied after migration), and the migration checklist (derive the new version from `v1/`, empty the previous version after migration). |
...
| File                                                       | What changes                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `intent-ops/README.md`                                     | Refresh for the versioned layout (`v1/flow`, `v1/spec`, `v1/cli`); generalize provenance wording (do not name the source project as a requirement); add package-layout diagram, version model (v1 current; future versions ship a changelog + migration rollout plan/instruction; previous versions emptied after migration), copy/promotion guidance, and pointer to `instructions.md`. |
...
- [ ] Links or references escaping the `v1/` scope would break future-version derivation — mitigate with a version-scope scan in Phase 4 and the migration checklist in `v1/README.md`.
- [ ] The package may drift from the source folders later; one-way extraction is intentional (user-confirmed 2026-08-27: `intent-ops` relocates to its own standalone repo; source folders stay project-local), and `instructions.md` will note that improvements should be authored in `intent-ops`.

## Recommended Architecture

### Principle 1: The package is self-contained and version-scoped

Every relative link resolves inside `intent-ops/`, and every version-scoped reference stays inside `v1/`. The package must not depend on sibling folders (`ai_oriented_kanban/`, `docs/` at repo root) at runtime — those exist only as extraction sources during this rollout. This is what keeps future-version derivation (and the confirmed version model: changelog + migration guide per version, previous versions emptied after migration) manageable.

### Principle 2: Versions are whole-folder snapshots

`v1/` (flow, spec, cli) is the complete current version; a future `v2/` is derived from `v1/` and ships a changelog of changes vs `v1/` plus a migration rollout plan/instruction doc (user-confirmed 2026-08-27). After migration, `v1/`'s flow/spec folders are emptied and `v2/` becomes the primary source of truth. No cross-version links, no partial-version references. `v1/README.md` is the manifest that documents the version model and makes the migration path explicit and verifiable.
...
Rules that govern behavior (docs-first retrieval flow, decision-evidence schema, simulation rubric, lane gates, ADR month-log convention) are stated as requirements. Machine-discoverable facts (repo map, routes, entrypoints, stack) are best-effort populated from the project scan; judgment-heavy examples are marked as `[project: fill in]` placeholders for human review and refinement (user-confirmed 2026-08-27), so no target project inherits another project's architecture.
...
- [x] Phase 7 — AI-ready docs reflection and next-plan handoff (follow-up plan created and consolidated; all open questions user-confirmed 2026-08-27 — see Resolved decisions)
...
- `v1/cli/init.md` is a **conformant rollout plan**: it follows the `rollout-plan-template.md` structure (Summary, Docs Impact with decision evidence, phased execution with verification gates, mandatory closing phases), and its phases generate **all folders and files inside `v1/spec/`** (entrypoint, ai, operations, adr, workflow, domain scaffolds); it includes a repo-scan phase that **best-effort populates** the generated spec from the project scan (machine-discoverable content — repo map, routes, entrypoints, stack — filled in, judgment-heavy sections left as `[project: fill in]` for human review and refinement; user-confirmed 2026-08-27), and verification gates (metadata fields, index registration, link scan, no-business-content scan).
- `v1/README.md` is non-empty and contains: what `v1/` contains (flow, spec, cli), the version model (v1 current; each future version ships a changelog vs the previous version + a migration rollout plan/instruction doc; previous versions' flow/spec folders are emptied after migration; the newest version is the primary source of truth), and the migration checklist (derive the new version from `v1/`, write changelog + migration guide, update package-root pointers, re-run validation, empty the previous version after migration).
...
- [ ] **1.3 — Write `v1/cli/init.md` as the genesis rollout plan**: author it per the `rollout-plan-template.md` — Summary; Docs Impact (spec files to generate, decision evidence, verification); phased execution whose phases generate **all folders and files inside `v1/spec/`** (entrypoint → ai → operations → adr/workflow → domain scaffolds), each phase with a verification gate; a repo-scan phase that produces a **best-effort populated spec** — AI fills in machine-discoverable content from the scan, judgment-heavy sections stay `[project: fill in]` for human review and refinement (user-confirmed 2026-08-27); mandatory closing phases (docs sync, reflection, drill, eval).
...
- [ ] **1.4 — Write `v1/README.md`**: version-scoped manifest — contents of `v1/` (flow, spec, cli), the version model (current v1; future versions ship a changelog vs the previous version + a migration rollout plan/instruction doc; previous versions emptied after migration; newest version is the primary source of truth — user-confirmed 2026-08-27), and the migration checklist (derive the new version from `v1/`, write the changelog + migration guide, update package-root pointers, re-run validation, empty the previous version after migration).
...
- Machine-discoverable sections (repo-map, routes, entrypoints, stack) are best-effort populated from the project scan; judgment-heavy sections retain `[project: fill in]` markers and the genesis plan includes a human review/refinement step (user-confirmed 2026-08-27).
...
- [ ] **2.2 — Execute genesis phase (AI layer)**: generalize `guide.md` (keep docs-first retrieval decision flow, post-task docs-update trigger, retrieval QA trigger; replace task-type index with one generic worked example + `[project: fill in]` placeholder entries), `assistant-context-index.md` (placeholder rows + registration instructions), `repo-map.md` (skeleton best-effort populated from the project scan — system boundaries, routes, entrypoints, stack — with judgment-heavy rows left as `[project: fill in]` for human review), `project-simulation-readiness.md` (keep full rubric/run-log; drop certifai refs), `_template.md` (generic source-of-truth placeholders).
...
- [ ] **3.3 — Generalize package READMEs + version model**: replace certifai delivery examples in `v1/flow/README.md` with `[project: describe your recent initiatives]` placeholders; refresh `intent-ops/README.md` (generic provenance "extracted from the AI-Oriented Kanban methodology and the spec-first docs system", versioned layout diagram, version model: v1 current / future versions ship a changelog + migration rollout plan/instruction with previous versions emptied after migration, pointer to `v1/README.md` migration checklist, copy + promotion guidance, navigator pointer).
...
- [ ] **4.2 — Version-scope scan**: verify that every `v1/`-rooted document references only targets inside `intent-ops/v1/` (or explicitly documented externals: extraction sources, promotion note).
  - **Independent verification**: scan output lists zero out-of-scope references; this is the gate that keeps future-version derivation a simple copy of a fully self-contained `v1/`.
...
- [ ] **6.6 — Record significant ADRs**: append dated entries to `v1/spec/adr/YYYY-MM.md` for (a) intent-ops package architecture (navigator + v1/flow + v1/spec + v1/cli, self-contained reference convention), (b) the versioned-layout decision (v1 current; each future version ships a changelog + migration rollout plan/instruction; previous versions emptied after migration; migration via `v1/README.md` — user-confirmed 2026-08-27), (c) the promotion path decision (`v1/spec/` ↔ `<project>/docs/`, host `docs/` untouched — user-confirmed 2026-08-27), (d) the recursive self-generation decision (`v1/cli/init.md` is a genesis rollout plan that generates all `v1/spec/` folders/files, best-effort populated from the project scan — user-confirmed 2026-08-27), (e) the HITL decision (human approval at each phase verification gate; tasks testable in isolation; pause-and-confirm loop defined in the rollout-plan template + `instructions.md`), (f) the backport/relocation decision (`intent-ops` moves to its own standalone repo; source folders stay project-local), per `v1/spec/adr/_template.md` + index.
...
**Progress**: `[x]` — follow-up plan authored and consolidated into this file (see the Follow-Up section below); handoff note recorded; all open questions user-confirmed 2026-08-27 (see Resolved decisions)

**Layer**: planning/documentation improvement layer

**Goal**: Capture what this rollout proved about the package, resolve open questions into owned decisions, and produce the next rollout plan (first real usage in a target project, v2 preparation, or relocation of the package to its own standalone repo).
...
- [ ] **7.3 — Author next rollout plan**: write the Follow-Up section of `v1/flow/10-plan/sculp-intent-ops.md` for the first real package usage (or v2 preparation / package relocation to its own standalone repo) — append in place, do not spawn a standalone follow-up file.
...
- Keep the extraction one-way: edits happen inside `intent-ops/`; the source folders (`ai_oriented_kanban/`, `docs/` at repo root) are read-only references for this rollout. (User-confirmed 2026-08-27: `intent-ops` will be relocated into its own standalone repo; the source folders are used only by the current project and are not reusable for new projects.)
...
- Version-scope rule: every reference inside `v1/` resolves inside `v1/`; the only documented externals are the extraction sources and the promotion note. This is what keeps future-version derivation a simple copy of a fully self-contained `v1/` (per the confirmed version model).
...
- `intent-ops/README.md` documents the versioned layout and the version model (per-version changelog + migration rollout plan/instruction doc; previous versions emptied after migration; newest version is the primary source of truth); `v1/README.md` contains the manifest, the migration checklist, and the changelog convention.
...
> **AI behavior at plan-creation time**: every ADR conflict found by the `### ADR Conflict Check` gate that is not explicitly resolved in the plan MUST be surfaced here for user confirmation. No conflicts were found.

### Resolved decisions (user-confirmed 2026-08-27)

All 14 open questions below were confirmed by the user on 2026-08-27; the question/answer items have been removed and each decision is now reflected in the plan body, the Decision Evidence Log, and the ADR month log (`v1/spec/adr/2026-08.md`). The purpose statement is aligned: `intent-ops` can be copied into any project; AI scans the project and builds a spec-first docs/spec folder `intent-ops/v1/spec` following spec-first-development best practices; users then plan and implement phased tasks from `intent-ops/v1/flow`, where each phased task is gated by a human verification step (HITL).

| #   | Question                                     | Confirmed decision                                                                                                                                                                                                                                                                                                               | Where reflected                                                                                 |
| --- | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 1   | Reference convention after copy              | Keep `intent-ops/v1/spec/` as canonical and document the promotion path to `<project>/docs/`. `v1/spec/` defers from any existing/user-defined docs because its files are registered and linked in `v1/spec/`, which is easier to navigate for spec-first (SDD) development.                                                     | ADR-0003; `instructions.md` first-copy guidance; Phases 1.2 / 3.3                               |
| 2   | Genesis plan generation scope                | Spec-only: `v1/flow/` is copied as-is, never generated — initially blank (like a task-management dashboard) with plans added by users as needed; any necessary target-project adjustments are documented in the genesis plan.                                                                                                    | ADR-0004; `v1/cli/init.md` output contract; Phase 1.3                                           |
| 3   | Domain scaffold set                          | Keep the 9-domain set (architecture, api, state, data, style, security, performance, testing, product) as-is; each is a lightweight `_template.md` and the set is immutable for all projects.                                                                                                                                    | Phase 2.5; Phase 2 verification gate; Decision Evidence Log                                     |
| 4   | Versioning cadence                           | Each future version (v2, v3, …) carries a changelog of what changed vs the previous version and a migration rollout plan/instruction doc (how to adapt existing docs to the new structure). After migration, the previous version's flow/spec folders/files are emptied and the new version becomes the primary source of truth. | ADR-0002; `v1/README.md` manifest + migration checklist; Summary, Principle 2, Success Criteria |
| 5   | Backport policy                              | `intent-ops` will be relocated into its own standalone repo; the source folders (`ai_oriented_kanban/`, `docs/`) are used only by the current project and are not reusable for new projects — no backport flow needed.                                                                                                           | Follow-Up Phase 3; Essential Implementation Details; Out of scope                               |
| 6   | Skills folder versioning                     | `skills/` stays package-root unversioned user content; skills can be copied/deleted/edited freely, are isolated and universal, and are not coupled with versions.                                                                                                                                                                | ADR-0005; `v1/README.md`                                                                        |
| 7   | HITL operational meaning                     | HITL = Human-In-The-Loop: a human reviews at each phase's verification gate, so users can test each phased task as it completes; task context is minimized so any team member can resume/continue/complete a task.                                                                                                               | Follow-Up Phase 2; `instructions.md` + rollout-plan template wording                            |
| 8   | Scan → build depth                           | Best-effort populated spec: AI fills in as much as possible from the scan results, leaving room for human review and refinement.                                                                                                                                                                                                 | `v1/cli/init.md` output contract (Phase 1.3); Phases 2.2 / 2.5; Principle 5; Success Criteria   |
| 9   | Spec-first best-practices inventory          | Confirmed complete: docs-first retrieval, ADR system + month-log, simulation readiness + docs-only drill, retrieval smoke tests, docs-maintenance protocol, workflow naming convention, per-domain scaffolds.                                                                                                                    | Phase 2 scaffold inventory; Phases 2.2–2.5                                                      |
| 10  | Where HITL lives in `v1/flow`                | Each phased task must be testable in isolation (focused feedback and iteration); the pause-and-confirm loop is defined in the rollout-plan template.                                                                                                                                                                             | Follow-Up Phase 2; rollout-plan template wording                                                |
| 11  | Applicability of "any project"               | "Any project" includes non-software contexts, but the initial focus is software projects (web development, mobile apps); other project types are a future-version concern.                                                                                                                                                       | Phase 2 domain set / v2 planning                                                                |
| 12  | `v1/spec` vs host project's existing `docs/` | Keep `v1/spec` as the new canonical source; do not touch the host's existing `docs/` or any other user-predefined docs folder.                                                                                                                                                                                                   | ADR-0003; `instructions.md` first-copy guidance; Follow-Up Phase 1                              |
...
- Version-scope scan: `skills/` references from `v1/`-rooted docs are a **documented package-root external** (same class as `README.md`/`instructions.md`); the scan whitelist in `instructions.md` was updated accordingly — future-version derivation stays a simple copy of a self-contained `v1/`.
...
- Next: execute the Follow-Up section of this file in a real project (all open questions confirmed by the user on 2026-08-27; see Resolved decisions).
- Blockers: none.
...
### Session Note — 2026-08-27 18:10 local

- Completed: open-question confirmation processing — the user answered all 14 open questions (2026-08-27); the plan was updated to reflect the answers (version model: per-version changelog + migration rollout plan/instruction, previous versions emptied after migration; genesis output contract: best-effort populated spec; HITL: human approval at each phase verification gate + tasks testable in isolation, pause-and-confirm defined in the rollout-plan template; backport: `intent-ops` relocates to its own standalone repo); a Resolved decisions table was added and the Q&A items were removed from `## Open Questions`.
- Verified by: manual diff review of the sections touched (Summary, Current Evaluation, Scope, Decision Evidence Log, Docs tables, Principles 2/5, Phases 1–2 gates, Phase 6.6, Success Criteria, Recommendation, Essential Implementation Details, Progress Dashboard, Handoff, Follow-Up).
...
1. **First real usage**: copy the package into a target project (or a scratch repo), execute `v1/cli/init.md` as a rollout plan (best-effort populated spec from the project scan, refined by a human), populate one domain scaffold with real content, and run one docs-first planning task through the flow (`10-plan` → `20-active` → `30-review`) with the HITL loop (human approval at each phase verification gate; each phased task testable in isolation — user-confirmed 2026-08-27).
2. **v2 preparation (optional)**: when the methodology changes accumulate, follow the confirmed version model (2026-08-27): derive `v2/` from `v1/`, ship a changelog of changes vs `v1/` plus a migration rollout plan/instruction doc, then empty `v1/`'s flow/spec folders so `v2/` becomes the primary source of truth (per `v1/README.md`).
3. **Backport decision — resolved (2026-08-27)**: `intent-ops` will be relocated into its own standalone repo; the origin folders (`ai_oriented_kanban/`, `docs/`) are used only by the current project and are not reusable for new projects, so no backport flow is needed. The relocation is executed when the package is extracted to its standalone repo.
...
**Verification gate**: plan file in `10-plan/` conforms to the template; decision evidence log complete; HITL loop exercised (human approval at each phase verification gate, each phased task testable in isolation — user-confirmed 2026-08-27); docs-only simulation drill passes with fallback ratio `<= 0.20`.

#### Phase 3: v2 prep or relocation execution

**Goal**: Execute the confirmed next-version action (2026-08-27): relocate `intent-ops` to its own standalone repo (source folders stay project-local), or prepare v2 per the confirmed version model.
