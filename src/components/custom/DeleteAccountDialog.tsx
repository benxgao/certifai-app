...
const DeleteAccountDialog: React.FC<DeleteAccountDialogProps> = ({ trigger }) => {
  ...
  // Call hook directly at top level - circular dependency prevention is handled in useAuthMutation
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Log deletion result to help with debugging
      console.log('Account deletion successful:', result);

      // Success is handled in the hook's onSuccess callback which triggers router.push('/signin')
      // Note: The page will navigate away before toastHelpers.success() completes
      toastHelpers.success.accountDeleted();

      // Close dialog (may not execute if page redirects)
      setIsOpen(false);
    } catch (error: unknown) {
      console.error('Account deletion failed:', error);
      setIsDeleting(false);
      const errorMessage = error instanceof Error ? error.message : String(error);
      toastHelpers.error.generic(errorMessage || 'Failed to delete account. Please try again.');
    }
  };
  ...
  return (
    <>
      {/* Page-level loading overlay shown during account deletion */}
      <PageLoader
        isLoading={isDeleting}
        text="Account is deleting..."
        variant="auth"
        showSpinner={true}
      />
